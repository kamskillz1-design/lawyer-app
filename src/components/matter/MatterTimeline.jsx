        ...
        return (
          <div key={i} className="card-soft p-4 space-y-3">
            <div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
              ...
              {Inbound ? "Cliente" : "Despacho"} · {c.channel} · {c.original_language}
              {c.translation_method === "ai" && <StatusBadge value="new" label="Traducción IA" />}
              {c.sensitivity !== "routine" && (
                <span className="flex items-center gap-1 text-red-700"><Gavel className="w-3 h-3" /> {sensitivityLabel(c.sensitivity)}</span>
              )}
            ...
            {c.reply_original && (
              <div className="rounded-xl bg-accent/60 p-3">
                <p className="text-xs text-muted-foreground mb-1 flex items-center gap-1"><MessageCircle className="w-3 h-3" /> Respuesta</p>
                <p className="text-sm">{c.reply_translation || c.reply_original}</p>
                {c.approved_by && <p className="text-xs text-muted-foreground mt-2">Aprobada por: {c.approved_by}</p>}
              </div>
            )}
          </div>
        );
      })}
      {!events.length && <p className="text-sm text-muted-foreground">Sin actividad registrada. Los mensajes del cliente y los cambios de etapa aparecerán aquí.</p>}
    </div>
  );
}